...
const blank2 = {text: 'select'};
const omega = {text:'omega', grade:'OM', pool: 7};
const alpha = {text:'alpha', grade:'Al', pool: 6};
const spr = {text:'super', grade:'S', pool: 5};
const a = {text:'A', grade:'A', pool: 4};
const b = {text:'B', grade:'B', pool: 3};
const c = {text:'C', grade:'C', pool: 2};
const d = {text:'D', grade:'D', pool: 1};
...
function abilityDropDown() {
  ...
  abilityChoice.addEventListener('change', function() {
    ...
    abilityDefense = abilityChoice.options[abilityChoice.selectedIndex].defense;
    abilityCost = abilityChoice.options[abilityChoice.selectedIndex].cost;
    gradeDropDown();
  })
//Loops through the abilityLibrary array and appends the data to the drop down.
  for (var i = 0; i < abilityLibrary.length; i++) {
      ...
      option.value = abilityLibrary[i].name;
...
function gradeDropDown() {
  ...
  gradeChoice.addEventListener('change', function() {
    gradeDropText = gradeChoice.options[gradeChoice.selectedIndex].text;
    gradeText = gradeChoice.options[gradeChoice.selectedIndex].grade;
    gradePool = gradeChoice.options[gradeChoice.selectedIndex].pool;
  })
  //Loops through the abilityLibrary array and appends the data to the drop down.
  for (var i = 0; i < gradePicks.length; i++) {
      var option = document.createElement("option");
      option.value = gradePicks[i].pool;
      option.text = gradePicks[i].text;
      select.appendChild(option);
  }
}
